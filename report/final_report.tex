...
%%  Generation
%%
%%  COMPILATION (requires aaai.sty, aaai.bst, fixbib.sty in same directory)
...
\pdfinfo{
  ...
  /Author (Taskia Maisha, Shadman Sakib)
}
...
\author{Taskia Maisha \\
  ID: 22201433 \quad Serial: 511 \\
  \And
  Shadman Sakib \\
  ID: 21301566 \quad Serial: 402 \\
  \AND
...
generates genre-conditioned sequences.  An optional bonus stage provides a
complete Reinforcement Learning from Human Feedback (RLHF) implementation---
including a human listening survey and reward model---with full training
execution left as future work.  Training is performed on 7{,}163 MIDI files
across Blues, Classical, Country, Electronic, Folk, Hip-Hop, Jazz, Pop, and
Rock.  All models use \texttt{BCEWithLogitsLoss} with positive class weighting
(pos\_weight\,$\approx$\,30) to address 97--98\% piano-roll sparsity.  The
Transformer achieves the best quantitative performance
(weighted BCE~Loss:~0.2193, Perplexity:~1.245, Rhythm~Diversity:~0.223),
approximately 4.6$\times$ lower loss than the AE (1.013) and VAE (1.020),
with strong nine-genre control.  A detailed analysis of VAE posterior
collapse and its observable metric effects is provided.  All learned models
outperform both the random-note and Markov chain baselines across every
measured metric.  A pre-RLHF human listening survey with 16 participants
evaluated Transformer outputs across six genres, yielding an average score
of 1.93/5.
\end{abstract}
...
\textbf{64 frames} (8 seconds).  A window is discarded if
...
(negative) and active (positive) cells.  We use \textbf{BCEWithLogitsLoss}
with a positive class weight $w^{+} \approx 15$--30 in place of unweighted
BCE or MSE\@.  The weight amplifies the gradient contribution of rare
active (note-on) cells, giving the model a strong training signal despite
the 97--98\% background of silence.  The exact value is computed per run as
$\min\!\bigl((1-p)/p,\;30\bigr)$ where $p$ is the observed
positive-cell rate in the training set.
...
\begin{equation}
  \hat{X} = g_\theta(z)
\end{equation}
Training minimises a positive-class-weighted binary cross-entropy loss:
\begin{equation}
  \Lcal_{\mathrm{AE}} = -\frac{1}{T}\sum_{t=1}^{T}\Bigl[
    w^{+}\,x_t\log\sigma(\hat{x}_t)
    + (1-x_t)\log\bigl(1-\sigma(\hat{x}_t)\bigr)
  \Bigr]
\end{equation}
where $w^{+}$ is the positive class weight and $\sigma$ is the sigmoid function.

\paragraph{Architecture.}
The \emph{encoder} is a 2-layer unidirectional LSTM with hidden
...
over the first 10 epochs, preventing the KL penalty from suppressing the
...
\begin{itemize}
  ...
  \item \emph{Positional embedding:} sinusoidal, maximum sequence length 512.
  \item \emph{Genre embedding:} a $9 \times 256$ learned lookup table;
        the genre token is prepended as the first token so the entire
  ...
  \item \emph{Transformer layers:} 4 decoder layers, each with 8-head
        ...
        feed-forward sublayer (inner dim 1024, ReLU activation).
  \item \emph{Output head:} linear projection $256 \to 88$ producing
        per-pitch logits; sigmoid applied at inference time.
\end{itemize}

\paragraph{Generation.}
Sequences are generated autoregressively using temperature-scaled sigmoid
sampling: $\hat{x}_t = \mathbf{1}\!\bigl[\sigma(h_t / T) > \tau\bigr]$
with $T = 0.7$ and threshold $\tau = 0.3$, applied over a sliding window
of 64 steps to keep the context in-distribution.
This temperature-threshold approach prevents the repetition loops
that arise under greedy ($\tau = 0.5$) decoding.
...
A lightweight reward model (a 3-layer MLP accepting a raw
$64 \times 88$ piano-roll segment and outputting a predicted
human-preference score in $[1,5]$) was implemented in
\texttt{src/models/reward\_model.py} and designed to be trained on
human survey responses (enabling reward estimation without running the
survey at every training iteration).  A 16-participant listening survey
was completed and the ratings are stored in
\texttt{outputs/survey\_results/pre\_rl\_scores.csv}.
Training the reward model and subsequent RLHF fine-tuning were
implemented in \texttt{src/training/train\_reward\_model.py} and
\texttt{src/training/train\_rlhf.py} respectively; execution of those
scripts is left as future work due to time constraints.
Rewards would be normalised per batch (subtract mean, divide by std)
to reduce REINFORCE variance.
...
Participants rated Task~3 Transformer output on a 1--5 scale
($1 = \text{poor},\;5 = \text{excellent}$) via a Google Form survey
distributed to 16 participants.  Six genre-specific samples were evaluated
(Blues, Classical, Country, Electronic, Folk, Hip-Hop).
Per-sample averages: Blues~2.19, Classical~2.13, Country~1.88,
Electronic~1.69, Folk~1.81, Hip-Hop~1.88.
The overall Task~3 mean is \textbf{1.93/5}.
Baselines and earlier tasks were not included in the survey.
...
\begin{table*}[ht]
  ...
  \caption{Quantitative evaluation of all models and baselines on the LMD
    ...
    BCE Loss is the positive-class-weighted \texttt{BCEWithLogitsLoss}
    (pos\_weight\,$\approx$\,30) reported from the best validation epoch.
    Perplexity ($\exp(\mathrm{BCE})$) is reported only for the Transformer,
    where it is the primary metric; ``--'' denotes not applicable.
    $\downarrow$\,lower is better; $\uparrow$\,higher is better.
    Human score from 16-participant survey of Task~3 only; baselines and
    Tasks 1--2 were not surveyed (N/A).
  ...
  \begin{tabular}{lcccccc c}
    ...
    \midrule
    Task 1: LSTM AE   & \underline{1.013} & --      & \textbf{0.249}
                      & \underline{0.093} & 0.421 & N/A & Single Genre \\
    Task 2: VAE       & 1.020   & --      & 0.089 & 0.000$^\dagger$ & \textbf{0.112}
                      & N/A & Moderate (9) \\
    Task 3: Transformer & \textbf{0.2193} & \textbf{1.245} & \underline{0.223}
                        & \textbf{0.053} & \underline{0.122} & \textbf{1.93/5} & Strong (9) \\
    \bottomrule
    \multicolumn{8}{l}{%
...
\begin{figure}[htbp]
  ...
  \caption{Task~2 VAE: training and validation loss.  The KL component
    collapsed to $\approx 0$ within the first five epochs despite KL
    annealing, leaving only the reconstruction term.}
...
unconstrained uniform sampling produces no rhythmic structure.
Its Pitch Sim.\ = 0.217 is coincidentally moderate: uniform pitch sampling
partially overlaps the real distribution by chance.
...
higher-order structure.  The Repetition Ratio (0.297) is the highest
across all models, confirming that the chain generates locally repetitive
motifs without long-range coherence.
...
approximation of the training pitch distribution.  The final weighted BCE
loss (1.013, measured with pos\_weight~$\approx$~30) reflects that the AE
has limited expressiveness on the highly imbalanced piano-roll data relative
to the Transformer.  The loss curve (Figure~\ref{fig:task1_loss}) shows
smooth convergence without overfitting, confirming that
\texttt{BCEWithLogitsLoss} with positive class weighting successfully
provides a gradient signal despite 97--98\% sparsity.

The AE's main limitations are (1) the absence of genre conditioning---it is
trained and sampled from a single latent distribution, providing no
mechanism to target specific genres at generation time---and
(2) a compressed fixed-length bottleneck ($d = 64$) that cannot fully
capture the temporal complexity of 64-step piano rolls.
...
schedule ($\beta: 0 \to 1$ over 10 epochs).  With
...
metric.  Its weighted BCE Loss of 0.2193 (Perplexity~1.245)
is approximately 4.6$\times$ lower than the AE (1.013) and 4.7$\times$
lower than the VAE (1.020), measured under the same
\texttt{BCEWithLogitsLoss} with pos\_weight~$\approx$~30.
Rhythm Diversity (0.223) and Repetition (0.053) indicate rhythmically
...
high.  Temperature-scaled sigmoid sampling ($T = 0.7$, threshold~0.3)
further prevents the repetition loops characteristic of greedy
decoding~\cite{huang2019musictransformer}.

Genre conditioning via the prepended genre token allows the model to
produce distinguishable output for all nine genres, confirmed by
per-genre listening and pitch-histogram inspection.

\subsection{Task 4: RLHF (Bonus)}

The RLHF infrastructure---policy gradient training loop, 3-layer reward
MLP accepting raw piano-roll input, and KL penalty against the original
checkpoint---is fully implemented in
\texttt{src/training/train\_rlhf.py} and
\texttt{src/training/train\_reward\_model.py}.
A 16-participant human listening survey was conducted on 6 genre-specific
Transformer outputs (Blues, Classical, Country, Electronic, Folk, Hip-Hop)
and the raw responses are archived in
\texttt{outputs/survey\_results/} with corresponding piano rolls in
\texttt{pre\_rl\_rolls.npy} and per-sample average scores in
\texttt{pre\_rl\_scores.csv}.
Running the reward model training script and the RLHF fine-tuning loop
are left as future work; accordingly, no before-vs-after comparison
or post-RLHF generated samples are included in this report.

\paragraph{Pre-RLHF survey results (Task~3 baseline ratings).}
The same 16 participants who would score the post-RLHF outputs also rated
six pre-RLHF Transformer samples.
Per-sample averages: Blues~2.19, Classical~2.13, Country~1.88,
Electronic~1.69, Folk~1.81, Hip-Hop~1.88; overall mean \textbf{1.93/5}.
These low ratings are consistent with the piano-roll representation's
inability to capture timbre or articulation: the MIDI-to-audio
conversion produces a mechanical acoustic-piano sound that lacks the
expressive qualities of real performances, reducing perceived musicality
independent of the model's generative capability.
...
\begin{enumerate}
  \item \textbf{The Transformer is the strongest model} across all
        quantitative metrics, achieving weighted BCE loss~0.2193
        (Perplexity~1.245)---approximately 4.6$\times$ lower than the AE
        (1.013) and VAE (1.020)---along with the best genre control over
        nine genres.  The AE achieves the highest rhythm diversity (0.249),
        suggesting richer note-duration variety in short reconstructed
        segments despite its inferior reconstruction fidelity.
  \item \textbf{VAE posterior collapse} renders the learned latent space
        ...
        interventions (hierarchical architecture, free
        bits~\cite{roberts2018musicvae}) are needed.
  \item \textbf{Class imbalance} (97--98\% sparse piano-rolls) is the
        primary training obstacle.  \texttt{BCEWithLogitsLoss} with
        positive class weighting (pos\_weight\,$\approx$\,30) is
        necessary for any stable learning signal.
  ...
  \item \textbf{RLHF infrastructure is complete} (policy gradient loop,
        reward MLP, KL-penalty anchor, 16-participant survey); executing
        the training pipeline remains as future work.
\end{enumerate}
