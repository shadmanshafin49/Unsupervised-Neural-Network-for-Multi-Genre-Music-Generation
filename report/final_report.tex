...
$\mathbf{64}$\textbf{ frames} (8 seconds).  A window is discarded if
...
(negative) and active (positive) cells.  We use \textbf{BCEWithLogitsLoss} with a positive class weight
$w^{+} \approx 15$--30 in place of unweighted BCE or MSE\@.  The
weight amplifies the gradient contribution of rare active (note-on)
cells, giving the model a strong training signal despite the 97--98\%
background of silence.  The exact value is computed per run as
...
high.  Temperature-scaled sigmoid sampling ($T = 0.7$, threshold~0.3) further
prevents the repetition loops characteristic of greedy
...
\begin{enumerate}
  ...
  \item \textbf{VAE posterior collapse} renders the learned latent space
        ...
        interventions (hierarchical architecture, free bits~\cite{roberts2018musicvae})
        are needed.
